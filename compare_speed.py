...
def measure_pymem(n):
    ...
    t_end = time()
    print(f"pyMem (Local: {vars(local_player)}): Memory: {t_end - t_start} sec")
    proc.close_process()
...
def measure_ctypes_mem(n):
    ...
    t_end = time()
    print(f"cTypes Mem (Local: {vars(local_player)}): Memory: {t_end - t_start} sec")
    proc.close()
...
def measure_ctypes_mem_struct(n):
    ...
    t_end = time()
    print(f"cTypes Mem Struct (Local: {vars(local_player)}): Memory: {t_end - t_start} sec")
    proc.close()
